...
from viper.core.archiver import Archiver, Compressor, Extractor, ZipExtractor
...
class TestExtractor:
    ...
    def test_extract(self, capsys, filename):
        assert os.listdir(os.getcwd()) == []
        instance = Extractor()
        res = instance.extract(
            archive_path=os.path.join(FIXTURE_DIR, filename), output_dir="."
        )
        assert res is True
    ...
    def test_extract_given_dir(self, capsys, filename):
        ...
        instance = Extractor()
        res = instance.extract(
            archive_path=os.path.join(FIXTURE_DIR, filename), output_dir="./foobar"
        )
        assert res is True
        assert instance.err is None
    ...
    def test_extract_password(self, capsys, filename):
        ...
        instance = Extractor()
        res = instance.extract(
            archive_path=os.path.join(FIXTURE_DIR, filename),
            output_dir=".",
            password="infected",
        )
        assert res is True
    ...
    def test_specified_cls_name_does_not_exist(self, capsys, filename):
        ...
        res = instance.extract(
            archive_path=os.path.join(FIXTURE_DIR, filename),
            output_dir=".",
            cls_name="ThisDoesNotExistExtractor",
        )
        assert res is False
        assert instance.err is not None
...
class TestCompressor:
    ...
    def test_compress(self, capsys, filename):
        assert os.listdir(os.getcwd()) == []
        instance = Compressor()
        res = instance.compress(
            file_path=os.path.join(FIXTURE_DIR, filename), archive_path="./"
        )
        assert res is True
        assert instance.err is None
    ...
    def test_compress_new_dir(self, capsys, filename):
        ...
        instance = Compressor()
        res = instance.compress(
            file_path=os.path.join(FIXTURE_DIR, filename), archive_path="./barfoo/"
        )
        assert res is True
        assert instance.err is None
        assert os.listdir(os.getcwd()) == ["barfoo"]
        assert instance.output_archive_path == "./barfoo/export.zip"
        assert os.listdir(os.path.dirname(instance.output_archive_path)) == [
            "export.zip"
        ]
    ...
    def test_specified_cls_name_does_not_exist(self, capsys, filename):
        ...
        res = instance.compress(
            file_path=os.path.join(FIXTURE_DIR, filename),
            archive_path="./",
            cls_name="ThisDoesNotExistCompressor",
        )
        assert res is False
        assert instance.err is not None
    ...
    def test_archive_no_extension(self, capsys, filename):
        ...
        instance = Compressor()
        res = instance.compress(
            file_path=os.path.join(FIXTURE_DIR, filename), archive_path="./foo"
        )
        assert res is False
        assert instance.err is not None
