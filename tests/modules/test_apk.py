...
class TestAPK:
    ...
    def test_cert(self, capsys, filename):
        ...
        assert re.search(
            r"SHA1: 65 2F 61 29 C8 7D 05 40 BF 98 6F C0 0E FD 9A B8 A7 87 84 DE", out
        )
        assert re.search(
            r"SHA256: 6E 56 64 27 DA 36 DD 91 36 39 B1 11 2F 74 7B 77 40 88 51 B4 85 7A 1D 63 EB F9 1E 02 B0 6F 20 88",
            out,
        )

    @pytest.mark.parametrize(
        "filename,pkg_name", [("hello-world.apk", "de.rhab.helloworld")]
    )
    ...
    @pytest.mark.skipif(
        sys.version_info < (3, 3), reason="Too slow on python2.7, makes travis fail."
    )
    @pytest.mark.skipif(
        sys.version_info >= (3, 3),
        reason="Uses way too much memory. Running the same commands in the client works fine...",
    )
