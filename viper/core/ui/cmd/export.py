...
class Export(Command):
    ...
    def __init__(self):
        ...
        self.parser.add_argument(
            ...
            "--zip",
            action="store_true",
            help="Export session in a zip archive (PW support: No)",
        )
        self.parser.add_argument(
            "-7",
            "--sevenzip",
            action="store_true",
            help="Export session in a 7z archive (PW support: Yes)",
        )
        self.parser.add_argument(
            "-p",
            "--password",
            action="store_true",
            help="Protect archive with a password (PW) if supported",
        )
    ...
    def run(self, *args):
        ...
        if not args.zip and not args.sevenzip:
            ...
            if os.path.isfile(store_path):
                self.log(
                    "error",
                    f'Unable to export file: file exists: "{store_path}"',
                )
                return
            ...
            if not os.path.isdir(args.value):
                try:
                    ...
                    self.log("error", f"Unable to export file: {err}")
                    return
            ...
            try:
                ...
                self.log("info", f"File exported to {store_path}")
        ...
        if res:
            self.log(
                "info",
                f"File archived and exported to {c.output_archive_path}",
            )
        else:
            self.log("error", f"Unable to export file: {c.err}")
