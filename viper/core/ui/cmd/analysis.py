...
class Analysis(Command):
    ...
    def __init__(self):
        ...
        group.add_argument(
            ...
            metavar="ANALYSIS ID",
            type=int,
            help="View the specified analysis",
        )
        group.add_argument(
            "-d",
            "--delete",
    ...
    def run(self, *args):
        ...
        if args.list:
            ...
            # Populate table rows.
            rows = [
                [analysis.id, analysis.cmd_line, analysis.stored_at]
                for analysis in analysis_list
            ]

            # Display list of existing results.
            self.log(
                "table", dict(header=["ID", "Cmd Line", "Saved On (UTC)"], rows=rows)
            )
            ...
            if result:
                self.log("info", f"[bold]Command line:[/bold] {result.cmd_line}")
                for line in json.loads(result.results):
                    self.log(line["type"], line["data"])
            else:
                self.log("info", f"There is no analysis with ID {args.view}")
