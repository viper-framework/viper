...
from viper.common.abstracts import (
    Command,
    Module,
...
def load_commands():
    ...
    for loader, cmd_name, ispkg in pkgutil.walk_packages(
        cmd.__path__, cmd.__name__ + "."
    ):
        ...
        try:
            ...
            print_warning(
                "Something wrong happened while importing the command {0}: {1}".format(
                    cmd_name, e
                )
            )
            continue
                ...
                if inspect.isclass(member_object):
                    ...
                    if (
                        ...
                        plugins[member_object.cmd] = dict(
                            ...
                            parser_args=get_argparse_parser_actions(
                                member_object().parser
                            ),
                            subparser_args=get_argparse_subparser_actions(
                                member_object().parser
                            ),
                        )
