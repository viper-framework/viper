...
class Functions(Module):
    cmd = 'functions'
    description = 'Detects user-defined functions'
    ...
    def run(self):
        ...
        # Get the file path for use by the functions.php script
        path = __sessions__.current.file.path
        
        # Call the functions.php script and store its output 
        try:
            output = subprocess.check_output(['php', '-f', viper_path + '/modules/functions.php', path])        
            f = open(path + '(raw)(functions)', 'w')        
            f.write(output)          
            f.close()
            print(output)
