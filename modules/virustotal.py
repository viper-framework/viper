...
class VirusTotal(Module):
    ...
    def __init__(self):
        ...
        self.parser.add_argument('-n', '--name', action='store_true', help='Determining the likely name of malware')
        self.parser.add_argument('-c', '--cluster', action='store_true', help='Strictly lexical clustering over malware-names')
    ...
    def run(self):
        ...
        if self.args is None:
            return   
        if self.args.cluster and not HAVE_CLUSTER:
            self.install()
            return
        ...
        # load virustotal report for the sample
        ...
        elif self.args.name or self.args.cluster:  # Determining the likely name of malware
            malwarenames = []
        ...
        elif self.args.submit: # Submit file
            if rows:
                self.log('', "")
